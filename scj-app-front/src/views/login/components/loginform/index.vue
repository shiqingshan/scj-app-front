<template>
    <div class="loginform">
      <div class="identity-tab">
        <n-tabs type="segment" style="width: 300px;" 
        ref="tabsInstRef" v-model:value="accountType"
        @update:value="handleUpdateValue">
          <n-tab name="1"> 求职 </n-tab>
          <n-tab name="2"> 招聘 </n-tab>
        </n-tabs>
      </div>
      <div class="form">
        <el-form 
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      loginStore.login(form.username, form.password, form.accountType)
      .then(() => {
        route.push('/home');
      })
      .catch(() => {
